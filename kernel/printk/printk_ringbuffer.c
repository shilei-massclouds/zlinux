...
#define BLK_DATALESS(blk) \
    (LPOS_DATALESS((blk)->begin) && LPOS_DATALESS((blk)->next))
...
static struct prb_data_block *
to_block(struct prb_data_ring *data_ring, unsigned long begin_lpos)
{
                   ...
                   unsigned long id)
{
    ...
    atomic_long_cmpxchg_relaxed(state_var, val_finalized, val_reusable);
}

/* Query the state of a descriptor. */
static enum desc_state get_desc_state(unsigned long id, unsigned long state_val)
{
                               ...
                               unsigned long *lpos_out)
{
    ...
    while ((lpos_end - lpos_begin) - 1 < DATA_SIZE(data_ring)) {
        ...
        d_state = desc_read(desc_ring, id, &desc, NULL, NULL);
...
get_next_lpos(struct prb_data_ring *data_ring,
                         ...
                         unsigned int max_size)
{
    ...
    if (BLK_DATALESS(&d->text_blk_lpos)) {
        ...
        r->text_buf = data_realloc(rb, r->text_buf_size, &d->text_blk_lpos, id);
    }
